...
pub async fn run_status_writer(state: BridgeState) {
    ...
    loop {
        ...
        let ds_connected = state.dsm.is_configured().await;
        let status = StatusData {
            bot_running: true,
            version: env!("CARGO_PKG_VERSION").to_string(),
            ds_connected,
            destinations: config.destinations.aliases.clone(),
        };
...
pub async fn run_config_watcher(state: BridgeState) {
    loop {
        ...
        for dir in CREDS_DIRS {
            let path = PathBuf::from(dir).join(CONFIG_UPDATE_FILE);
            if !path.exists() { continue; }
            log::info!("Config update detected in {}", dir);
            ...
            if let Some(t) = update.get("bot_token").and_then(|v| v.as_str()) {
                if !t.is_empty() && !t.contains("...") { config.telegram.bot_token = t.to_string(); }
            }
            if let Some(u) = update.get("authorized_users").and_then(|v| v.as_array()) {
                let p: Vec<u64> = u.iter().filter_map(|v| v.as_u64()).collect();
                if !p.is_empty() { config.telegram.authorized_users = p; }
            }
            if let Some(d) = update.get("destinations").and_then(|v| v.as_object()) {
                let mut aliases = std::collections::HashMap::new();
                for (k, v) in d {
                    if let Some(s) = v.as_str() { aliases.insert(k.clone(), s.to_string()); }
                }
                if !aliases.is_empty() { config.destinations.aliases = aliases; }
            }
            if let Err(e) = config.save(&state.config_path) { log::error!("Save failed: {}", e); }
            else { log::info!("Config saved"); }
        }
    }
}
